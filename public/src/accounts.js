...
function sortAccountsByLastName(accounts) {
  ...
  return accounts.sort((previousAccount, currentAccount) =>
    previousAccount.name.last.toLowerCase() < currentAccount.name.last.toLowerCase() ? -1 : 1);
...
function getAccountFullNames(accounts) {
  ...
  // let res = accounts.map((account) => `${account.name.first} ${account.name.last}`);
  let res = accounts.map(({ name }) => `${name.first} ${name.last}`);
  return res;
}
